...
from axipy import provider_manager, Schema
...
class ConverterEngine:
    ...
    def _safe_open(filepath):
        """Открывает файл, при необходимости копируя его во временную папку.

        Провайдер DWG Аксиомы не умеет открывать файлы по путям
        с не-ASCII символами (кириллица и т.п.). В этом случае
        файл копируется во временную папку с простым именем.

        Возвращает (data_object, temp_path или None).
        """
        has_non_ascii = any(ord(c) > 127 for c in filepath)
        ...
        temp_path = os.path.join(
            tempfile.gettempdir(),
            'cad2gis_{}{}'.format(uuid.uuid4().hex[:8], ext))
        shutil.copy2(filepath, temp_path)

        try:
            obj = provider_manager.open({'src': temp_path})
    ...
    def _safe_schema(data_object):
        """Читает schema из data_object. При ошибке возвращает пустую Schema().

        Некоторые провайдеры (DWG как RasteredTable) не отдают schema —
        для них создаётся пустая схема, чтобы экспорт шёл без атрибутов.
        """
        try:
    ...
    def prescan(self, files, output_dir, output_format, split_layers):
        ...
        Для неразбитого режима файлы НЕ открываются — прескан мгновенный.
        """
        # ── Быстрый путь: без разбивки по слоям ──
        if not split_layers:
            predictions = []
        ...
        # ── Разбивка по слоям: открываем файлы для получения слоёв ──
        predictions = []
        total_units = 0
        layer_cache = {}
    ...
    def _try_extent_from_obj(obj):
        ...
        # Пробуем вызвать как метод (без аргументов)
    ...
    def _rect_to_tuple(val):
        ...
        # Через атрибуты xmin/xmax/ymin/ymax
        for pat in [
            ('xmin', 'xmax', 'ymin', 'ymax'),
        ...
        # Через индексы: 4 элемента (xmin, ymin, xmax, ymax)
        try:
        ...
        # Rect из двух Point: (xmin, ymin) (xmax, ymax)
        try:
        ...
        try:
            ...
            try:
                if src_schema is not None:
                    # count — метод у Schema, вызываем его
                    if callable(getattr(src_schema, 'count', None)):
                        schema_has_cols = src_schema.count() > 0
                    elif hasattr(src_schema, '__len__'):
                        schema_has_cols = len(src_schema) > 0
            except Exception:
                schema_has_cols = False

            if not schema_has_cols:
                self._log('  Схема недоступна — экспорт без атрибутов')
