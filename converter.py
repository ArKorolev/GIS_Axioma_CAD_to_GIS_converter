...
class ConverterEngine:
    ...
    def scan_extents(files):
        ...
        for filepath in files:
            ...
            # Путь 1: provider_manager.open → coordsystem.rect
            if extent is None:
                try:
                    data_object = provider_manager.open({'src': filepath})
                    try:
    ...
    def _rect_to_tuple(val):
        ...
        # Через индексы: Rect печатается как (xmin ymin) (xmax ymax)
        # Возможно это tuple-like (xmin, ymin, xmax, ymax)
        try:
    ...
    def _replace_crs_in_mif_bytes(self, mif_path, crs):
        ...
        for i, line in enumerate(lines):
            if line.strip().lower().startswith(b'coordsys'):
                try:
                    new_bytes = new_line.encode('latin-1')
                except Exception:
        ...
        if not found:
            for i, line in enumerate(lines):
                if line.strip().startswith(b'Delimiter'):
                    try:
                        new_bytes = new_line.encode('latin-1')
                    except Exception:
                        new_bytes = new_line.encode('utf-8')
                    lines.insert(i + 1, new_bytes)
                    found = True
                    break
    ...
    def _convert_one(self, filepath, output_dir, output_format,
        ...
        definition = {'src': filepath}
        data_object = provider_manager.open(definition)
        output_files = []
        src_schema = data_object.schema
    ...
    def _export(self, items, out_path, output_format,
        ...
        try:
            ...
            if output_format == 'tab' and crs is not None:
                ...
                raise ValueError(
                    'Неподдерживаемый формат: {}'.format(output_format))

        finally:
            self._cleanup_tab_files(temp_tab_path)

    def _tab_via_mif(self, temp_tab_path, final_tab_path, crs):
        temp_mif_path = temp_tab_path.rsplit('.', 1)[0] + '_mif.mif'
