...
class ConverterEngine:
    ...
    def _convert_one(self, filepath, output_dir, output_format,
        ...
        src_schema = self._safe_schema(data_object)
        schema_has_cols = False
        if src_schema is not None:
            try:
                names = list(src_schema.attribute_names)
                schema_has_cols = len(names) > 0
            except Exception:
                schema_has_cols = False

        try:
            if schema_has_cols:
                # БЫСТРЫЙ ПУТЬ: schema работает (AVIA.DWG)
                output_files = self._convert_layers_fast(
                    data_object, output_dir, output_format,
                    basename, src_schema, crs)
            else:
                # МЕДЛЕННЫЙ ПУТЬ: schema недоступна (_04.ПЗУ.ГЧ.DWG)
                self._log('  Схема недоступна — группировка по атрибуту Layer')
                output_files = self._convert_layers_grouped(
                    data_object, output_dir, output_format,
                    basename, crs)
        finally:
            data_object.close()
    ...
    def _convert_layers_fast(self, data_object, output_dir, output_format,
                             basename, src_schema, crs):
        """Быстрый путь: items(layer_name) работает, schema доступна.
    ...
    def _convert_layers_grouped(self, data_object, output_dir, output_format,
                                    basename, crs):
        """Медленный путь: items() без фильтра, группировка по Layer.

        Возвращает список созданных файлов.
        """
        # 1. Читаем все объекты одним проходом
        try:
            all_items = list(data_object.items())
        except Exception as e:
            self._log('  Ошибка чтения items(): {}'.format(e))
            return []

        if not all_items:
            self._log('  Нет объектов для конвертации')
            return []

        self._log('  Всего объектов: {}'.format(len(all_items)))

        # 1b. После items() повторяем попытку получить нативную схему
        #     (до чтения данных data_object.schema может падать,
        #      но после items() — работает и сохраняет типы атрибутов)
        native_schema = None
        try:
            ...
            self._log('  Нативная схема недоступна, используем string-схему')

        # 2. Группируем по атрибуту Layer
        groups = {}
        for feature in all_items:
            ...
            if layer_name not in groups:
                groups[layer_name] = []
            groups[layer_name].append(feature)

        self._log('  Слоёв с данными: {}'.format(len(groups)))

        # 3. Для каждой группы: схема + экспорт
        output_files = []
        ...
        for layer_name, features in groups.items():
            ...
            # Используем нативную схему, если доступна.
            # Иначе — строим string-схему из реальных объектов (fallback).
            if native_schema is not None:
                schema = native_schema
            else:
                schema = self._build_schema_from_features(features, data_object)

            attr_count = len(list(schema.attribute_names))
            self._log('  Слой "{}": {} объект(ов), {} атрибут(ов)'.format(
                layer_name, len(features), attr_count))
            ...
            try:
                self._ensure_output_writable(out_path, output_format)
                self._export(iter(features), out_path, output_format,
                             schema, safe_name, crs)
                output_files.append(out_path)
            except Exception as e:
                self._log('  Ошибка экспорта слоя "{}": {}'.format(
                    layer_name, e))
